...
{
    //Static int/char/string/boolean will exist through the whole program.
    //Static variables/constants will keep its data in memory to be able to be reused.
    ...
    static char symbolChoice; 			//Symbol choice (X or O)
    static char playAgain; 				//(Y or N)
    static int enterNum = 0; 			//Player choice (1-9)
    static boolean complete = false; 	//Completion of a game.
    ...
    static int gamesWon, gamesLost, gamesPlayed, gamesTied, winner, counter; //These variables generate the game results.
    ...
    public static void main (String[] args)
    {
    	...
    	//Welcome Message
		c.println("\t \t \t \t \t \t Samger's Tic Tac Toe Game");
		gameDescription(); //print out game description
	    c.print("\n \n \n \n \t \t \t \t \t Please enter your choice to be either X or O: ");
	    
		//While user does not type in a valid X or O.
		while (!((symbolChoice == 'X') || (symbolChoice == 'x') || (symbolChoice == 'O') || (symbolChoice == 'o')))
		{
		    symbolChoice = c.readChar();
		    c.clear();
		    c.print("\n \n \n \n \t \t \t \t \t Please enter your choice to be either X or O: ");
		}
		
		if ((symbolChoice == 'X') || (symbolChoice == 'x'))
		{
		    while (!complete) //Runs until game is complete.
		    {
		...
		else if ((symbolChoice == 'o') || (symbolChoice == 'O'))
		{
		    while (!complete) //Runs until game is complete.
		    {
		...
		if (winner == player)
		{
		    ...
		    gamesWon++; //Number of games won by human player is added
		}
		...
		else if ((playAgain == 'n') || (playAgain == 'N')) //Player wants to end game, results shown.
		{
		    ...
		    c.println("\n \t \t \t \t \t \t You have won " + gamesWon + " game(s).");
		    c.println("\t \t \t \t \t \t You have lost " + gamesLost + " game(s).");
		    c.println("\t \t \t \t \t \t You have tied " + gamesTied + " game(s).");
		    c.println("\t \t \t \t \t \t You have played " + gamesPlayed + " game(s).");
	
		    //Information about program
		    c.println("\n\n \t \t \t \t \t \t Samger's Tic Tac Toe Game");
		    c.print("\n \t \t \t \t \t \t \t Version 1.0");
		    c.print("\n \t \t \t \t \t \t Programmed by Samger Li");
		}
    ...
    { 
		...
		if (winner == 0)
		{
		    complete = true; //Game is completed.
		    for (x = 0 ; x < square ; x++)
		    {
				if (intSquare [x] == 0)
				{
    ...
    {
		symbolChoice = ' '; //symbol choice is reprompted to user
		playAgain = ' ';
		for (x = 0 ; x < square ; x++)
		{
		...
		complete = false; //all values are reset for a new game
		counter = 0;
		winner = 0;
    ...
    { 
		c.print("\n \t \t You would think that Tic-Tac-Toe is a straightforward game; in fact you'll discover that this");
		c.print("\n \t \t game isn't quite as simple as you think! Tic-Tac-Toe involves looking ahead and trying to");
		c.print("\n \t \t figure out what the computer might do next. You must come up with a strategy to beat the computer");
		//Print out Rules of the game.
		c.print("\n \n \t \t \t \t \t \t \t Rules");
		c.print("\n \n \t \t 1. The game is played against the computer on a 3 squares by 3 squares grid.");
		c.print("\n \t \t 2. You have the option to play as X or O.");
    ...
    } //End game description method
    
} // SamgerTicTacToe class
